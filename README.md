...
> The project is based off a story game where each decision made by the user changes the story in some way for different results. The user would be able to first choose character, weapon, rank, and nationality. Through entering certain keywords as directed by the game, the user will be able to start the game and make choices throughout the game as they face decisions in the story line (which would look closer to a Directed Acylic Graph than an actual "line"). Additionally, as players progress, a leveling system with clout will influence how the character is treated within the storyline. The character's clout will dynamically impact interactions and events. The story line would diverge depending on the decisions that had been taken, and depending on the user's attributes (character, weapon, rank, etc. listed above). The character could be promoted, demoted, imprisoned, court martialed, being taken POW depending on decisions they take throughout the story --- after each of these cases, the player would have a chance to fix the situation. However, if the character was killed, the game would end. An endgame screen would appear, giving a summary of how far the user had come. In addition, there's also an option to increase or decrease the difficulty, which would make key events easier or harder. For example, if the character is being court martialed, in hard mode it might require the character to become an high ranking officer to survive the ordeal. On easy mode, it might merely require the character to have enough clout.
